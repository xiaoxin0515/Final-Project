...
def negamax(is_ai, depth, alpha, beta):
    ...
    order(blank_list)  # 有邻居的优先 # Ordering search order to Improve pruning efficiency
...
def order(blank_list):
    ...
    for item in blank_list:
        for i in range(-1, 2):  # -1,0,1 看当前位置的上下左右四个位置是否为空
            for j in range(-1, 2):
                if i == 0 and j == 0:
                    continue
                if (last_pt[0] + i, last_pt[1] + j) in blank_list:
                    blank_list.remove((last_pt[0] + i, last_pt[1] + j))
                    blank_list.insert(0, (last_pt[0] + i, last_pt[1] + j))  # 0是index，即把有邻居的位置放到blank list的最前面
...
#def main():
    ...
    # for i in range(0, 5):  ## simulate the game 100 times in a program
    #     new = process()
    #     if (new == 1):
    #         white_win += 1
    #     elif (new == -1):
